...
Of the ssGSEA algorithm version v4 is used. Gene set databases are from MSigDB version v6.1.

### 2.2 Summary of ssGSEA calculation results
...
* thresholds of p value and FC

## 4. Results of differential enrichment analysis

```{r diff enrichment results, echo=F}
if(params$ContinChoice_gsea == FALSE | ClinColClasses(params$mainParameter) == "factor") {
  message("Gene sets called significant are changed in ", contrastGroup1, " when compared to ", contrastGroup2,".")
} else {
  message("Gene sets called significant are changed with regard to ", mainParamter, ".")
  if(params$filter == TRUE){
    message("Only samples included in ", params$filtergroups, " were included into the analysis.")
  }
}
if(!is.null(params$covariates)){
  message(params$covariates, " were included into the analysis as covarites.")
}
